...
class MediaAnalyticsExpert:
    ...
    def _get_column(self, df: pd.DataFrame, metric: str) -> Optional[str]:
        ...
        Prioritizes columns that actually have data (non-zero sum).
        """
        if metric.lower() in self.COLUMN_MAPPINGS:
            ...
            # If multiple candidates, pick the one with data
            if len(candidates) > 1:
    ...
    def _calculate_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Campaign-level metrics - DYNAMIC AGGREGATION
        # Use dynamic campaign column if available
        agg_key = campaign_col if campaign_col else 'Campaign_Name'
        if agg_key in df.columns:
            agg_dict = {}
            if spend_col: agg_dict[spend_col] = 'sum'
            if conv_col: agg_dict[conv_col] = 'sum'
            if impr_col: agg_dict[impr_col] = 'sum'
            if clicks_col: agg_dict[clicks_col] = 'sum'
            if 'ROAS' in df.columns: agg_dict['ROAS'] = 'mean'
            if 'CPA' in df.columns: agg_dict['CPA'] = 'mean'

            try:
                campaign_metrics = df.groupby(agg_key).agg(agg_dict).round(2)
                ...
                if clicks_col: rename_map[clicks_col] = 'Clicks'
                
                campaign_metrics = campaign_metrics.rename(columns=rename_map)
                
                metrics["by_campaign"] = campaign_metrics.to_dict('index')
            except Exception as e:
                logger.warning(f"Campaign groupby failed: {e}")
        ...
        if 'Platform' in df.columns:
            try:
                ...
                if agg_dict:
                    ...
                    platform_metrics = platform_metrics.rename(columns=rename_map)
...
Focus on recommendations that can be implemented immediately with clear tactical steps."""
    ...
    def get_rag_summary(self, data: pd.DataFrame) -> Dict[str, Any]:
        """Get RAG-enhanced summary for the given data.
        
        Args:
            data: Campaign data DataFrame
            
        Returns:
            Dict containing brief and detailed summaries + insights
        """
        # 1. Identify key topics from data
        topics = ["campaign optimization"]
        if 'ROAS' in data.columns:
            avg_roas = data['ROAS'].mean()
            # Handle possible float/int issues
            try:
                if avg_roas < 3.0:
                    topics.append("improving ROAS")
                else:
                    topics.append("scaling high ROAS campaigns")
            except:
                topics.append("general campaign performance")
                
        # 2. Fetch external knowledge
        external_context = ""
        try:
            # Lazy load reasoning engine if not already initialized
            ...
            # Fetch insights for top topic
            if topics:
                knowledge_result = self.reasoning_engine.get_knowledge_enhanced_insights(
                    campaign_summary="Auto-analysis in progress",
                    topic=topics[-1]
                )
                if isinstance(knowledge_result, dict):
                     # Extract raw knowledge chunks if available, or the synthesized response
                     external_context = knowledge_result.get('response', '')
        except Exception as e:
            logger.warning(f"Failed to fetch RAG context: {e}")
            external_context = "External knowledge unavailable."

        # 3. Generate Analysis with Knowledge Injection using standard pipeline
        return self.analyze_all(data, external_knowledge=external_context)

    def _generate_executive_summary(self, metrics: Dict, insights: List, recommendations: List, external_knowledge: str = "") -> Dict[str, str]:
...
Then "DETAILED:" then the detailed summary."""
        ...
        if self.use_anthropic and self.anthropic_api_key:
            try:
                ...
                msg = f"Claude Sonnet failed: {str(e)}"
        ...
        if not combined_response and self.gemini_client:
            try:
                ...
                msg = f"Gemini 2.5 Pro failed: {str(e)}"
                logger.warning(f"❌ {msg}")
                errors.append(msg)
        
        # 3. Try GPT-4o-mini (fallback 2)
        # Fix: Check self.client explicitly regardless of use_anthropic flag to ensure fallback works
        if not combined_response and self.client:
            try:
                ...
                msg = f"GPT-4o-mini failed: {str(e)}"
                logger.warning(f"❌ {msg}")
                errors.append(msg)
        
        if not combined_response:
            logger.error(f"❌ ALL LLM calls for summary generation failed! Errors: {'; '.join(errors)}")
...
=== ANALYSIS PHILOSOPHY (AVINASH KAUSHIK STYLE) ===

1. THE "SO WHAT?" TEST: For every metric you cite, ask "So What?" If you can't explain the business impact, delete it.
2. OUTCOMES OVER ACTIVITY: Focus on Revenue, ROAS, and CPA. Impressions and CTR are only relevant if they explain conversion performance.
3. QUANTIFY IMPACT: Never say "improved" - say "improved by 23 percent, generating $4.2K incremental revenue".
4. ROOT CAUSE ANALYSIS: Apply "5 Whys" methodology to dig beyond symptoms.
5. PRESCRIPTIVE ACTIONS: "Optimize targeting" is banned. Say "Exclude 'Unknown' age group to save $1.2K/month" instead.
6. NO FLUFF: Be direct, professional, and brutal with the truth. No corporate speak.
...
Generate the complete BRIEF and DETAILED RAG-enhanced analysis now:"""
    ...
    def _prepare_enhanced_summary_data(self, metrics: Dict, insights: List, recommendations: List) -> Dict:
        ...
        if campaign_metrics:
            ...
            for campaign, m in campaign_metrics.items():
                camp_data = {
                    'name': str(campaign)[:50],  # Truncate long names
                    'spend': m.get('spend', 0),
    ...
    def _generate_executive_summary_with_rag(self, 
        ...
        Args:
            ...
            **kwargs: Additional context like campaign_objective, conversion_definition, etc.
        ...
        logger.info(f"Context args: {list(kwargs.keys())}")
        ...
        try:
            ...
            # Enrich context with explicit objective if provided
            campaign_objective = kwargs.get('campaign_objective')
            if campaign_objective:
                logger.info(f"Enriching context with explicit objective: {campaign_objective}")
                campaign_context['summary'] += f" The explicit campaign objective is '{campaign_objective}'."
                campaign_context['goals'].insert(0, {
                    'description': str(campaign_objective),
                    'priority': 'high',
                    'type': 'user_defined',
                    'urgency': 'immediate'
                })
            ...
            # Fallback to OpenAI
            # Fix: Check self.client explicitly to ensure fallback works even if openai_api_key env var is missing but client exists
            if not llm_response and (self.openai_api_key or self.client):
                try:
                    ...
                    if not openai_client.api_key:
                        logger.warning("⚠️ OpenAI Fallback: No API Key found in client or env var.")
                    else:
